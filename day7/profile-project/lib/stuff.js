$(document).ready(function () {
  $('#links_group').width($('#main').width());
  $('.links').width($('#main').width()/2);
  ...
  $('#main').animate({ opacity: '0' }, 12000, 'swing', function () {
...
function bgMovement (reverse) {
  var size = reverse ? "1350px" : "3000px";
  $('body').animate({ "background-size" : size}, 60000, 'swing');
  bgMovement(!reverse);
}
